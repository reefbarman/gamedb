...
namespace GameDBEditorLibrary
{
    internal class DictionaryValueComponent : Component
    {
        ...
        private void RenderPopup(int windowID)
        {
            m_scrollPos = EditorGUILayout.BeginScrollView(m_scrollPos);
            {
                ...
                if (m_editable && GUILayout.Button("Save & Close"))
                {
                    ...
                    {
                        if (dictEntry.Key != null && !dictEntry.Key.Equals(FieldBase.NullRefToken) && !dict.ContainsKey(dictEntry.Key))
                        {
        ...
        {
            return GameDBLibrary.JsonSerialization.Serialize(DictionaryTypeUtils.SerializeValue(GetField().GetTypeArg<DictionaryType>(), GetValue()));
        }
    }
}
