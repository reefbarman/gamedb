...
namespace GameDBLibrary
{
    ...
    public class GameDBBase
    {
...
#if !FREE_VERSION
        /// <summary>
        /// <strong>(Pro Version only)</strong><br/> 
        /// Imports the GameDB from a server. Can be used in conjuction with the Golang server provided in the (Pro version)
        ...
        public RequestUpdater ImportFromServer(string serverHost, string downloadHost, string userID, string tag, string originalJSON, string cachePath, bool binary, string key, string salt, string[] columImportList = null, Action<Exception> onImport = null)
        {
            var checksum = string.Empty;
            var baseGameDBJson = string.Empty;
            var gameDBJson = string.Empty;

            cachePath = Path.Combine(cachePath, $"gamedb/{tag}/{ScopeName}.gamedb");
            ...
            if (File.Exists(cachePath))
            {
                try
                {
                    ...
                    gameDBJson = BinaryGameDB.Deserialize(cacheBytes, key, salt);
            ...
            {
                baseGameDBJson = gameDBJson = originalJSON;
            }

            checksum = Utils.GetChecksum(Encoding.UTF8.GetBytes(gameDBJson));

            return Remote.GetLatestDeployment(serverHost, downloadHost, ScopeName, tag, checksum, userID, key, salt, (error, json, revision) => {
                if (error == null)
                {
                    if (json != null)
                    {
                        ...
                        try
                        {
                            var cachable = BinaryGameDB.Serialize(gameDBJson, key, salt);

                            if (cachable != null)
                            {
                                Directory.CreateDirectory(Path.GetDirectoryName(cachePath));
                                File.WriteAllBytes(cachePath, cachable);
                            }
                        }
                        catch (Exception e)
                        {
