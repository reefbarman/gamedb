...
#if FREE_VERSION
            typeNames = new[] {KeyType.@string.ToString()};
#endif
...
#if FREE_VERSION
            var freeTypes = new System.Collections.Generic.List<string>(typeNames);
            freeTypes.Remove(FieldType.@enum.ToString());
            freeTypes.Remove(FieldType.tableRef.ToString());
            freeTypes.Remove(FieldType.dictionary.ToString());
            typeNames = freeTypes.ToArray();
#else
