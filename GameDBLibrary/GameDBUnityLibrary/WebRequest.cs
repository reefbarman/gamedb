            ...
            switch (method)
            {
                ...
                case RequestMethod.GET:
                    if (requestParams != null)
                    {
                        ...
                        {
                            paramPairs[i] = string.Format("{0}={1}", WWW.EscapeURL(requestPair.Key), WWW.EscapeURL(requestPair.Value));
                            i++;
                        }

                        if (paramPairs.Length > 0)
                        {
                            url += string.Format("?{0}", string.Join("&", paramPairs));
                        }
                    }
        ...
        private RequestUpdater DoRequest(UnityWebRequest request, Action<Exception, IDownloadHandler> callback)
        {
            ...
            onUpdate = delegate () {
                if (result.isDone)
                {
                    ...
                    if (request.isError)
                    {
                        callback(new Exception(string.Format("Error in request - statusCode: {0} error: {1}", request.responseCode, request.error)), null);
                    }
                    else
                    {
